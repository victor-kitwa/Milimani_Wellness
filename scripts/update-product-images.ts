...
// Load .env.local the same way seed.ts / drizzle.config.ts do, before
...
/*
 * Temporary placeholder product photos sourced from Unsplash, keyed by
 * exact product name (matches the `name` column, not the seed image slug,
 * since a couple of those slugs don't match slugify(name)). Swap these out
 * for real product photography when it's ready.
 */
...
async function main() {
  ...
  for (const [name, url] of Object.entries(IMAGE_MAP)) {
    ...
    await db.update(products).set({ images: [url] }).where(eq(products.name, name));
